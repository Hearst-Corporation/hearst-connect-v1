...
// Le tier `featured` cerclé est le rendu officiel du bloc : on n'y touche pas.
        ...
        <div className="isolate mx-auto mt-10 grid max-w-md grid-cols-1 gap-8 lg:mx-0 lg:max-w-none lg:grid-cols-3">
          {tiers.map((tier) => (
            <div
              ...
              className="group/tier rounded-3xl p-8 ring-1 ring-zinc-200 data-featured:bg-zinc-900 data-featured:ring-zinc-900 xl:p-10 dark:bg-zinc-800/50 dark:ring-white/10 dark:data-featured:bg-zinc-800/50 dark:data-featured:ring-2 dark:data-featured:ring-accent-500"
            >
              <h3
                id={`tier-${tier.id}`}
                className="text-lg/8 font-semibold text-zinc-950 group-data-featured/tier:text-white dark:text-white dark:group-data-featured/tier:text-accent-400"
              >
                {tier.name}
              </h3>
              <p className="mt-4 text-sm/6 text-zinc-600 group-data-featured/tier:text-zinc-300 dark:text-zinc-300">
                {tier.description}
              </p>
              {typeof tier.price === 'string' ? (
                <p className="mt-6 text-4xl font-semibold tracking-tight text-zinc-950 group-data-featured/tier:text-white dark:text-white">
                  {tier.price}
                </p>
              ) : (
                <>
                  <p className="mt-6 flex items-baseline gap-x-1 group-not-has-[[name=frequency][value=monthly]:checked]/tiers:hidden">
                    <span className="text-4xl font-semibold tracking-tight text-zinc-950 group-data-featured/tier:text-white dark:text-white">
                      {tier.price.monthly}
                    </span>
                    <span className="text-sm/6 font-semibold text-zinc-600 group-data-featured/tier:text-zinc-300 dark:text-zinc-400">
                      /mois
                    </span>
                  </p>
              ...
              <a
                ...
                className="mt-6 block w-full rounded-md bg-accent-600 px-3 py-2 text-center text-sm/6 font-semibold text-white shadow-xs group-data-featured/tier:bg-white/10 group-data-featured/tier:inset-ring group-data-featured/tier:inset-ring-white/5 hover:bg-accent-500 group-data-featured/tier:hover:bg-white/20 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-accent-600 group-data-featured/tier:focus-visible:outline-white/75 dark:bg-accent-500 dark:shadow-none dark:hover:bg-accent-400 dark:focus-visible:outline-accent-500"
              >
                {tier.cta}
              </a>
              <ul
                role="list"
                className="mt-8 space-y-3 text-sm/6 text-zinc-600 group-data-featured/tier:text-zinc-300 xl:mt-10 dark:text-zinc-300"
              >
                {tier.features.map((feature) => (
                  <li key={feature} className="flex gap-x-3">
                    <CheckIcon
                      aria-hidden="true"
                      className={clsx(
                        tier.featured ? 'text-white dark:text-accent-400' : 'text-accent-600 dark:text-accent-400',
                        'h-6 w-5 flex-none',
                      )}
                    />
                    {feature}
