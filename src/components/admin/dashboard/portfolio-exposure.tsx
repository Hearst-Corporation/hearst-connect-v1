...
function StrategyDetail({ row }: Readonly<{ row: AdminExposureStrategy }>) {
  ...
  const body = (
    <div className={clsx(surfaceInset, 'p-4')}>
      <p className="text-sm font-semibold text-zinc-950 dark:text-white">{row.strategyLabel}</p>
      ...
      <TabList className="grid grid-cols-2 gap-2 @[28rem]:grid-cols-3 @[52rem]:grid-cols-6">
        {rows.map((row) => (
          ...
          >
            ...
            <span className="mt-2 text-xs font-semibold text-zinc-950 dark:text-white">{row.strategyLabel}</span>
            <span className="mt-0.5 text-lg font-semibold tabular-nums text-zinc-950 dark:text-white">
              {formatPercent(row.actualBps, { fromBps: true })}
            </span>
            <span className="mt-0.5 text-[11px] text-zinc-500">target {formatPercent(row.targetBps, { fromBps: true })}</span>
          </Tab>
        ))}
      </TabList>
