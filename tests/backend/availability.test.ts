...
describe('availabilityFromResolu + measuredCount (F-05)', () => {
  ...
  it('un champ STALE avec valeur ne badge jamais live', () => {
    ...
    const count = measuredCount(availabilityFromResolu(bloc, endpoint))
  ...
  it('un champ LIVE avec valeur badge live', () => {
    const bloc = { status: 'LIVE', value: [{ id: '1' }] }
    const count = measuredCount(availabilityFromResolu(bloc, endpoint))
    expect(signalOf(count)).toBe('live')
  })

  it('une valeur absente reste indisponible', () => {
    const bloc = { status: 'UNAVAILABLE', value: null, reason: 'events_unreachable' }
    const count = measuredCount(availabilityFromResolu<readonly { id: string }[]>(bloc, endpoint))
